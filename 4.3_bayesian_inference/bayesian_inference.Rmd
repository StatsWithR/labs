...
In this lab we will review exploratoray data analysis using the `ggplot2` 
package for data visualization, which is included in the `tidyverse`. The main 
focus of this lab is to be able to obtian and interprete credible intervals and 
hypothese tests using Bayesian methods for numerical variables. The data and 
...
function can be too long and difficult to visually parse and interpret if the 
...
the condition `premie` equals "full term" is met.
...
constructing confidence intervals and conducting hypothesis tests. The following 
...
The second argument, `data`, specifies the dataset `nc.fullterm` that contains 
...
`prior_family` options, we need to specify prior hyperparamters. For `JZS`, 
...
argument `mu_0 = 7.7`. We will use the default argument `rscale = 1`. We 
alsospecify that we are looking for the 95% credible interval by setting 
`cred_level = 0.95`. The method of inference can be either 
...
option as there are no closed form results. Finally, the argument `cred_level` 
specifies the probability for the credible interval, with a default of 0.95. 
...
To conduct this hypothesis test, we will need to change the `type` arguement to 
...
save to the object `weight.post`:
...
In particular, `samples` is a matrix which contain the draws from the MCMC 
simulation, where the columns `mu` and the column `sig2` are posterior samples 
...
```{r}
quantile(samples[["mu"]], c(0.025, 0.5, 0.975)) 
quantile(samples[["y.pred"]], c(0.025, 0.5, 0.975))
```
...
provides greater power to detect a difference fue to the intervention. If we 
