...
<div class="question">
Use the new value of $k$ to calculate the posterior probability of being an 
outlier. Which observation has a posterior probability of being an outlier 
that exceeds the prior probability of being an outlier of 
`r round(1 - prob_obs_not_outlier, 4)`?
...
In the code above, the indices for the model start with $0$ the intercept, so 
we need to add one to extract the variable names. We can compare this to the 
Highest probability model that we found earlier and the 
...
<div class="question">
Using the reduced data, what covariates are included in **all** of the best 
...
<div class="question">
Using the reduced data, a 95% prediction interval for the individual who is 
predicted to have the highest predicted wages under `BPM` would be.
